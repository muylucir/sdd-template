...
#### 테스팅 요구사항
- `design.md`에 나열된 **모든** 정확성 속성에 대해 property-based test 작성
- 명시된 대로 정확히 property test에 태그 추가: `[주석 문법] Feature: [기능명], Property {번호}: {속성 설명}`
- 명시된 최소 반복 횟수로 property test 실행
- 엣지 케이스와 에러 시나리오에 대한 단위 테스트 작성
- 완전한 워크플로우에 대한 통합 테스트 작성
...
#### 테스팅 원칙
- 기능을 구현하면서 **동시에** 테스트 작성 (나중이 아님)
- Property test는 테스트하는 서비스/모듈 직후에 작성해야 함
...
#### 테스트 작성하기
1. Property-based 테스트:
   - 각 정확성 속성에 대해 작성
   - tech.md의 테스팅 프레임워크 사용
   - 필수 태그/주석 추가
   - 명시된 반복 횟수로 실행
   - 속성이 성립하는지 확인
...
3. 통합 테스트:
   - 완전한 워크플로우 테스트
   - End-to-end 시나리오 테스트
   - 사용자 스토리가 예상대로 작동하는지 확인

#### 작업을 완료로 표시하기 전
- [ ] 코드가 작성되고 모든 가이드라인을 따름
- [ ] 모든 파일이 올바른 위치에 있음
- [ ] 명명 규칙을 따름
- [ ] 모든 테스트(property, unit, integration)가 통과함
- [ ] 린터 에러 없음
- [ ] 타입 체킹 통과 (해당되는 경우)
- [ ] 수용 기준이 충족됨
- [ ] 요구사항이 만족됨
...
[Property 16: 접근을 위한 인증 필수에 대한 property test 작성]
[테스트 태그: // Feature: meeting-room-reservation, Property 16: 접근을 위한 인증 필수]
[fast-check으로 테스트 실행, 100회 반복]
[테스트 통과]

[tasks.md 업데이트: 작업 4.1을 [x] 완료로 표시]

개발자: 작업 4.1 완료. 모든 테스트 통과. 작업 4.2로 이동합니다.
```
...
- ✅ design.md의 모든 정확성 속성이 테스트를 통과함
